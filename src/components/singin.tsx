...
export default function Login({ onSignUp, onBack, onAuthSuccess }: LoginProps) {
  ...
  return (
    ...
    <div className="relative w-full flex flex-col items-center justify-start font-sans overflow-hidden min-h-screen">
      {/* Header */}
      <header className="w-full flex flex-row justify-between items-center px-8 lg:px-16 py-8 z-20">
        {/* Logo */}
        <div
          style={{
      ...
      <div className="relative w-full max-w-sm p-6 space-y-6 bg-white dark:bg-black rounded-lg border border-zinc-200 dark:border-zinc-800 shadow-lg dark:shadow-zinc-900/50">
        ...
        {onBack && (
          <button
            type="button"
            onClick={onBack}
            className="mb-4 inline-flex items-center gap-2 text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors"
          >
        ...
        <div className="text-center space-y-2">
          <p className="text-sm text-zinc-600 dark:text-zinc-400">
            ...
            <button 
              type="button"
              onClick={onSignUp}
              className="font-medium text-zinc-900 dark:text-zinc-50 underline underline-offset-4 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors cursor-pointer"
            >
              Sign up
            </button>
          </p>
